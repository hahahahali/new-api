...
| `common/email_templates.go` | 7类多语言邮件模板（zh/en/ja/fr/ru/vi）：注册验证码、密码重置、达人申请验证码、审核通过、审核拒绝、打款通知、KOL 欢迎邮件。**v7 重写（2026-05）**：全部函数升级为 v7 HTML 设计（渐变背景、白色圆角卡片、base64 内嵌 logo、共用 helper 函数 `emailWrapper/emailBadge*/emailBtn/emailInfoBox/emailWarningBox/emailStatCard*/buildCodeDigitCells`）；语言由 `normalizeEmailLang` 归一，支持 zh/en/ja/fr/ru/vi，`es` fallback 到 en。`<head>` 内置 `format-detection`/`x-apple-disable-message-reformatting`/`color-scheme:light only` 等 iOS Mail 兼容 meta；`emailStatCard3` 用扁平表格 + `height` HTML 属性强制三栏等高，规避 iOS 暗色模式下 div 嵌套被剥样式的问题；`stat2Val` 各语言均采用 ≤4 字符短格式（`10天/10日/10j/10д/10ng/10d`）避免移动端窄列换行错位。新增 helper `appendLangQuery(url, code)` 用于给链接追加 `?lng=` / `&lng=`（保留 `#fragment`）；`BuildKolWelcomeEmail` 控制台按钮链接强制追加 `?lng=en`，让从邮件入口进入的用户固定看到英文控制台。`BuildKolWelcomeEmail` 注册成功后由 `controller/user.go` 异步触发。 |
