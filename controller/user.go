...
// It also copies the language preference from the affiliate application to the user.
func applyKolInviteToken(token string, userId int) error {
	if err := model.ConsumeKolInviteToken(token, userId); err != nil {
		return err
	}
	// Copy lang from the affiliate application so emails are sent in the right language
	if app, err := model.GetApplicationByToken(token); err == nil && app != nil && app.Lang != "" {
		_ = model.DB.Model(&model.User{}).Where("id = ?", userId).Update("lang", app.Lang).Error
	}
	return nil
}
