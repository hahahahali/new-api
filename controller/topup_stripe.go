...
func (*StripeAdaptor) RequestPay(c *gin.Context, req *StripePayRequest) {
	...
	topUp, _, err := service.CreatePendingTopUpWithKolRebate(id, req.Amount, originalMoney, referenceId, model.PaymentMethodStripe, model.PaymentProviderStripe)
	if err != nil {
	...
	if err != nil {
		...
		_ = topUp.Update()
		logger.LogError(c.Request.Context(), fmt.Sprintf("Stripe 获取支付链接失败 user_id=%d trade_no=%s error=%q", id, referenceId, err.Error()))
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "拉起支付失败"})
		return
	}
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Stripe 充值订单创建成功 user_id=%d trade_no=%s amount=%d money=%.2f", id, referenceId, req.Amount, topUp.Money))
	c.JSON(http.StatusOK, gin.H{
		"message": "success",
		"data": gin.H{
			"pay_link": payLink,
...
func fulfillOrder(ctx context.Context, event stripe.Event, referenceId string, customerId string, callerIp string) {
	...
	if topUp := model.GetTopUpByTradeNo(referenceId); topUp != nil {
		if err := service.ProcessCommission(topUp.UserId, topUp.TradeNo, topUp.Money); err != nil {
			logger.LogError(ctx, fmt.Sprintf("Stripe 充值佣金处理失败 trade_no=%s error=%q", referenceId, err.Error()))
		}
	}
