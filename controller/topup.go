	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		if topUp.Status == common.TopUpStatusPending {
			...
			if err := service.ProcessCommission(topUp.UserId, verifyInfo.ServiceTradeNo, topUp.Money); err != nil {
				logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 回调佣金处理失败 trade_no=%s error=%q", verifyInfo.ServiceTradeNo, err.Error()))
			}
		}
	} else {
...
func RequestAmount(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"message": "success", "data": strconv.FormatFloat(discountedMoney, 'f', 2, 64), "original": strconv.FormatFloat(payMoney, 'f', 2, 64), "rebate_rate": rebateRate})
}
